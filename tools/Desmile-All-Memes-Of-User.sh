...
printf "Desmiling All Smiled Memes...\n\n"
...
while IFS= read -r id <&3 && IFS= read -r is_smiled <&4; do
    ...
    if [[ "$is_smiled" = "true" ]]; then
      resp=$(curl -s -H "authorization: Bearer $bearertoken" -H 'ifunny-project-id: iFunny' -X DELETE "https://api.ifnapp.com/v4/content/$id/smiles")
