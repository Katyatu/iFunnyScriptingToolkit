...
if [ $# -ne 1 ]; then
    printf "\nUsage:\n  ./Deunsmile-All-Memes-Of-User.sh 'creator_name'\n\nPurpose:\n  To deunsmile all of the selected user's posted memes.\n\nNote:\n  Ensure you are using SINGLE quotes '' and NOT double quotes \"\" to encapsulate the selected username.\n\n"
    skipcleanup="true"
    exit 1
fi
...
############################
# Deunsmile All Unsmiled #
############################

printf "Deunsmiling All Unsmiled Memes...\n\n"
...
while IFS= read -r id <&3 && IFS= read -r is_unsmiled <&4; do
    ...
    if [[ "$is_unsmiled" = "true" ]]; then
      resp=$(curl -s -H "authorization: Bearer $bearertoken" -H 'ifunny-project-id: iFunny' -X DELETE "https://api.ifnapp.com/v4/content/$id/unsmiles")
