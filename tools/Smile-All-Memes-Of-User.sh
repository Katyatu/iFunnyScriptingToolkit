...
########################
# Smile All Not Smiled #
########################

printf "Smiling All Not Smiled Memes...\n\n"
total_lines=$(wc -l < $WORKDIR/ids.txt)
line_num=0
while IFS= read -r id <&3 && IFS= read -r is_smiled <&4; do

    ((line_num++))
    echo -n "[${line_num}/${total_lines}] "

    if [[ "$is_smiled" = "false" ]]; then
      resp=$(curl -s -H "authorization: Bearer $bearertoken" -H 'ifunny-project-id: iFunny' -X PUT "https://api.ifnapp.com/v4/content/$id/smiles")
      ...
      echo "$id already smiled"
    fi

done 3<$WORKDIR/ids.txt 4<$WORKDIR/is_smiled.txt